...
{
  UserToPMABufferCopy(Data_Pointer, ENDP4_TXADDR, Data_Len);

  SetEPTxCount(ENDP4, Data_Len);
...
void Set_CSW (uint8_t CSW_Status, uint8_t Send_Permission)
{
  ...
  UserToPMABufferCopy(((uint8_t *)& CSW), ENDP4_TXADDR, CSW_DATA_LENGTH);

  SetEPTxCount(ENDP4, CSW_DATA_LENGTH);
  Bot_State = BOT_ERROR;
  if (Send_Permission)
  {
