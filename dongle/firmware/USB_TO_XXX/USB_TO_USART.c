...
void USB_TO_USART_ProcessCmd(uint8* dat, uint16 len)
{
	...
	while(index < len)
	{
		...
		case USB_TO_XXX_INIT:
			...
			buffer_out[rep_len++] = USB_TO_USART_NUM;
		...
		case USB_TO_XXX_STATUS:
			buffer_out[rep_len++] = USB_TO_XXX_OK;
			buffer_out[rep_len++] = (uint8)(asyn_tx_idx >> 0);
			buffer_out[rep_len++] = (uint8)(asyn_tx_idx >> 8);
			buffer_out[rep_len++] = (uint8)(asyn_rx_idx >> 0);
			buffer_out[rep_len++] = (uint8)(asyn_rx_idx >> 8);
