...
void USB_TO_SWD_ProcessCmd(uint8_t *dat, uint16_t len)
{
	...
	while(index < len)
	{
		...
		case USB_TO_XXX_INIT:
			if (ERROR_OK == app_interfaces.swd.init(device_idx))
			{
		...
		case USB_TO_XXX_CONFIG:
			if (ERROR_OK == app_interfaces.swd.config(device_idx, dat[index + 0], 
		...
		case USB_TO_XXX_FINI:
			if (ERROR_OK == app_interfaces.swd.fini(device_idx))
			{
				buffer_reply[rep_len++] = USB_TO_XXX_OK;
			}
			else
		...
		case USB_TO_SWD_SEQOUT:
			...
			if (ERROR_OK == app_interfaces.swd.seqout(device_idx, &dat[index + 2], bitlen))
			{
				buffer_reply[rep_len++] = USB_TO_XXX_OK;
			}
			else
		...
		case USB_TO_SWD_SEQIN:
			bitlen = GET_LE_U16(&dat[index]);
			if (ERROR_OK == app_interfaces.swd.seqin(device_idx, &buffer_reply[rep_len + 1], bitlen))
			{
				buffer_reply[rep_len] = USB_TO_XXX_OK;
			}
			else
		...
		case USB_TO_SWD_Transact:
			...
			if (ERROR_OK == app_interfaces.swd.transact(device_idx, dat[index], &swd_data, 
