...
U8 rc;
   ...
   (void)bdmcf_tx_msg(BDMCF_CMD_WAREG+commandBuffer[3]);   // Send the command word
...
U8 f_CMD_CFVx_READ_REG(void) {
   ...
   returnSize  = 5;
   (void)bdmcf_tx_msg(BDMCF_CMD_RAREG+commandBuffer[3]);    /* send the command */
...
U8 rc;
   ...
   (void)bdmcf_tx_msg(BDMCF_CMD_WDMREG+commandBuffer[3]);  // Send the command word
...
U8 f_CMD_CFVx_READ_DREG(void) {
   ...
   returnSize  = 5;
   (void)bdmcf_tx_msg(BDMCF_CMD_RDMREG+commandBuffer[3]);   // send the command
   return bdmcf_rx(2,commandBuffer+1);
}
...
U8 i;
   ...
   i = (commandBuffer[3]>>3)+((commandBuffer[3]&0x07)!= 0);
...
U8 f_CMD_JTAG_READ_WRITE(void) {
   ...
   jtag_read_write(commandBuffer[2], commandBuffer[3], commandBuffer+4);
   (void)memcpy(commandBuffer+1, commandBuffer+4, commandBuffer[2]); // relocate data to correct location
   return BDM_RC_OK;
}
